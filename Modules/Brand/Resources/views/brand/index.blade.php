        ...
        <div class="table-content table-basic ecommerce-components product-list">
            <div class="card">
                <div class="card-body">
                    ...
                    <div
                        ...
                        @if (hasPermission('brand_create'))
                            <div class="align-self-center d-flex gap-2">
                                <!-- add btn -->
                                <div class="align-self-center">
                                    <a href="{{ route('brand.create') }}" role="button" class="btn-add"
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['brands'] as $key => $brand)
                                    <tr>
                                        <td>{{ @$brand->id }}</td>
                                        <td>
                                            <img src="{{ showImage(@$brand->iconImage->paths['50x200'], 'default-1.jpeg') }}"
                                                alt="img" width="35px">
                                        </td>
                                        <td>
                                            {{ statusBackend(@$brand->status->class, $brand->status->name) }}
                                        </td>
                                        <td>{{ @$brand->serial }}</td>
                                        <td class="create-date">{{ showDate(@$brand->created_at) }}</td>
                                                    ...
                                                    @if (hasPermission('brand_update'))
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('brand.edit', $brand->id) }}"><span
                                                                    class="icon mr-12"><i
                    ...
                    <!--  table end -->
                    <!--  pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['brands']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
