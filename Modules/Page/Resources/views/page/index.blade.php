        ...
        <div class="table-content table-basic ecommerce-components product-list">
            <div class="card">
                <div class="card-body">
                    ...
                    <div
                        ...
                        @if (hasPermission('page_create'))
                            <div class="align-self-center d-flex gap-2">
                                <!-- add btn -->
                                <div class="align-self-center">
                                    <a href="{{ route('pages.create') }}" role="button" class="btn-add"
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['pages'] as $key => $page)
                                    <tr>
                                        <td>{{ @$page->id }}</td>
                                        <td>{{ Str::limit(@$page->title, 20) }}</td>
                                        <td>{{ Str::limit(@$page->slug, 20) }}</td>
                                        <td>
                                            {{ statusBackend(@$page->status->class, $page->status->name) }}
                                        </td>
                                        <td class="create-date">{{ showDate(@$page->created_at) }}</td>
                                                    ...
                                                    @if (hasPermission('slider_update'))
                                                        <li>
                                                            <a class="dropdown-item"
                                                                href="{{ route('pages.edit', $page->id) }}"><span
                                                                    class="icon mr-12"><i
                    ...
                    <!--  table end -->
                    <!--  pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['pages']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
