        ...
        <div class="table-content table-basic ecommerce-components product-list">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['contacts'] as $key => $contact)
                                    <tr>
                                        <td>{{ @$key + 1 }}</td>
                                        <td>{{ @$contact->name }}</td>
                                        <td>{{ @$contact->email }}</td>
                                        <td>{{ @$contact->subject }}</td>
                                        <td class="create-date">{{ showDate(@$contact->created_at) }}</td>
                                            ...
                                            <div class="dropdown dropdown-action">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    ...
                                                    @if (hasPermission('slider_update'))
                                                        <li>
                                                            <button class="dropdown-item main-modal-open"
                                                                data-url="{{ route('admin.contacts.details', $contact->id) }}"><span
                                                                    class="icon mr-12"><i
                                                                        class="fa-solid fa-eye"></i></span>
                                                                {{ ___('common.View') }}</button>
                                                        </li>
                                                    @endif
                                                </ul>
                                            </div>
                    ...
                    <!--  table end -->
                    <!--  pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['contacts']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
