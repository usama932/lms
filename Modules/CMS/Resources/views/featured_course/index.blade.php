            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div
                        ...
                        <!-- add btn start -->
                        @if (hasPermission('featured_course_create'))
                            <div class="align-self-center d-flex gap-2">
                                <!-- add btn -->
                                <div class="align-self-center">
                                    <button data-url="{{ route('admin.featured-course.create') }}" role="button"
                        ...
                        <table class="table table-bordered">
                            <thead class="thead">
                                <tr>
                                    ...
                                    <th>{{ ___('course.Category') }}</th>
                                    <th>{{ ___('course.Instructor') }}</th>
                                    <th>{{ ___('course.Section') }}</th>
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['courses'] as $key => $feature)
                                    <tr>
                                        <td>{{ @$key + 1 }}</td>
                                        <td>{{ Str::limit(@$feature->course->title, 20) }}</td>
                                        <td> {{ @$feature->course->category->title }}</td>
                                        <td>
                                            {{ @$feature->course->instructor->name }}
                                        </td>

                                        <td>
                                            {{ ___('course.Total Section') }} :
                                            {{ numberFormat(@$feature->course->sections->count()) }}
                                            <br>
                                            {{ ___('course.Total Lesson') }} :
                                            {{ numberFormat(@$feature->course->lessons->count()) }}
                                        </td>

                                        <td>
                                            @if (@$feature->course->is_free)
                                                {{ ___('common.Free') }}
                                            @else
                                                {{ showPrice(@$feature->course->price) }}
                                            @endif
                                        </td>
                                        <td>
                                            {{ statusBackend(@$feature->status->class, $feature->status->name) }}
                                        </td>
                                        ...
                                        <td class="action">
                                            <div class="dropdown dropdown-action">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @if (hasPermission('featured_course_update'))
                                                        <li>
                                                            <a class="dropdown-item main-modal-open"
                                                                href="javascript:void(0);"
                                                                data-url="{{ route('admin.featured-course.edit', $feature->id) }}">
                                                    ...
                                                    @endif
                                                    @if (hasPermission('featured_course_delete'))
                                                        <li>
                                                            <a class="dropdown-item delete_data" href="javascript:void(0);"
                                                                data-href="{{ route('admin.featured-course.destroy', $feature->id) }}">
