        ...
        <div class="table-content table-basic ecommerce-components product-list ">
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['incomes'] as $key => $income)
                                    <tr>
                                        <td>{{ @$key + 1 }}</td>
                                        <td>
                                            {{ @$income->transaction->account->ac_name }}
                                        </td>
                                        <td>
                                            {{ @$income->transaction->user->name }}
                                        </td>
                                        <td>
                                            {{ @$income->note }}
                                        </td>
                                        <td>
                                            {{ showPrice(@$income->amount) }}
                                        </td>
                                        <td>
                                            {{ showDateTime(@$income->created_at) }}
                                        </td>
                                        <td>
                                            {{ statusBackend(@$income->status->class, $income->status->name) }}
                                        </td>
                                    </tr>
                                @empty
                                    <!-- empty table -->
                    ...
                    <!-- end table data -->
                    <!-- pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['incomes']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
