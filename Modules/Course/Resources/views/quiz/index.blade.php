    ...
    <div class="page-content">
        ...
        @include('backend.ui-components.breadcrumb', [
            ...
            'routes' => [
                ...
                route('course.curriculum.index', $data['section']->course_id) => ___('course.Course Section List'),
                '#' => @$data['title'],
            ],
            'buttons' => 0,
        ])
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div
                        ...
                        <!-- add btn start -->
                        @if (hasPermission('course_quiz_create'))
                            <div class="align-self-center d-flex gap-2">
                                <!-- add btn -->
                                <div class="align-self-center">
                                    <button
                                        onclick="mainModalOpen(`{{ route('course.quiz.create', [$data['section']->id]) }}`)"
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tbody class="tbody">
                                ...
                                @forelse ($data['quizzes'] as $key => $quiz)
                                    <tr>
                                        <td>{{ @$key + 1 }}</td>
                                        <td>{{ Str::limit(@$quiz->title, 30) }}</td>
                                        <td>{{ Str::limit(@$quiz->section->title, 30) }}</td>
                                        <td>
                                            {{ @$quiz->marks }}
                                        </td>
                                        <td>
                                        ...
                                        <td class="action">
                                            <div class="dropdown dropdown-action">
                                                ...
                                                <ul class="dropdown-menu dropdown-menu-end">
                                                    @if (hasPermission('course_quiz_update'))
                                                        <li>
                                                            <a class="dropdown-item"
                                                                onclick="mainModalOpen(`{{ route('course.quiz.edit', $quiz->id) }}`)"
                                                    ...
                                                    @if (hasPermission('course_quiz_delete'))
                                                        <li>
                                                            <a class="dropdown-item delete_data" href="javascript:void(0);"
                                                                data-href="{{ route('course.quiz.destroy', $quiz->id) }}">
                    ...
                    <!-- end table data -->
                    <!-- pagination start -->
                    @include('backend.ui-components.pagination', ['data' => $data['quizzes']])
                    <!--  pagination end -->
                </div>
            </div>
        </div>
