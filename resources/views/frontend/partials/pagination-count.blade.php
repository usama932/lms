...
@if ($paginator->hasPages())
    <div class="pagination mt-30">
        ...
        <ul class="pagination-list mb-10">
            ...
            @else
                <li><a class="page-number {{ @$event }}" href="{{ $paginator->previousPageUrl() }}"
                        rel="prev"><i class="ri-arrow-left-s-fill"></i></a></li>
            @endif
            ...
            @foreach ($elements as $element)
                ...
                @if (is_array($element))
                    @foreach ($element as $page => $url)
                        ...
                        @else
                            <li><a href="{{ $url }}"
                                    class="page-number {{ @$event }}">{{ $page }}</a></li>
                        @endif
                    @endforeach
                @endif
            @endforeach
            ...
            @if ($paginator->hasMorePages())
                <li><a class="page-number {{ @$event }}" href="{{ $paginator->nextPageUrl() }}"
                        rel="prev"><i class="ri-arrow-right-s-fill"></i></a></li>
            @else
